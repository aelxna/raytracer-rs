...
pub fn trace_ray<'a>(
    ...
    for bs in bspheres {
        ...
        let constant: f32 = (r.origin - bs.center).sq_mag() - (bs.radius * bs.radius);
        ...
        // intersection -> check shapes in sphere
        if (t1 > ERR_BOUND) || (t2 > ERR_BOUND) {
            for &i in &bs.shape_indices {
                ...
                match &shapes[i] {
                    Shape::Sphere(sp) => {
                        ...
                        let constant: f32 =
                            (r.origin - sp.center).sq_mag() - (sp.radius * sp.radius);
