...
fn main() -> Result<()> {
    ...
    let progress = ProgressBar::new((scene.width * scene.height) as u64).with_style(style);
    progress.enable_steady_tick(Duration::from_millis(100));

    let img = RgbImage::from_par_fn(scene.width, scene.height, |x, y| {
        // create ray pointing to that pixel
        let loc: Vec3 = ds.ul + (ds.dx * (x as f32)) + (ds.dy * (y as f32));
        let r: Ray3 = Ray3::new(
            scene.eye + ((loc - scene.eye) * 0.01),
            (loc - scene.eye).norm(),
        );
        let mut stack: Vec<f32> = Vec::new();
        stack.push(scene.eta);
